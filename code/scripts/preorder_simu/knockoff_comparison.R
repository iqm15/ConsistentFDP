...
param.list <- fromJSON(file = "config_files/knockoff_comparison.json") ##
#-------------------------------------------------------------------------------
...
for (amplitude in param.list$amplitude_grid){
  ...
  for (s in param.list$sparsity_grid) {
    ...
    for (r in 1:param.list$nb_run){
      ...
      KR_bound <- sapply(1:length(binary_pvalues), preorder_KR, proc_s, lambda, delta, binary_pvalues, return_Vk = TRUE)
      KR_U_bound <- sapply(1:length(binary_pvalues), preorder_KR_U, proc_s, lambda, delta, binary_pvalues, return_Vk = TRUE)
      ...
      for (j in 1:length(binary_pvalues)) {
        KR_bound_interp <- c(KR_bound_interp, get_interpolated_FDP_bound(j, KR_bound[1:j], nb_rej[1:j])$fdp_interp)
        KR_U_bound_interp <- c(KR_U_bound_interp, get_interpolated_FDP_bound(j, unlist_KR_U_bound$KR_U_bound[1:j], nb_rej[1:j])$fdp_interp)
      }

      # computeLi and Goeman KJI bounds
